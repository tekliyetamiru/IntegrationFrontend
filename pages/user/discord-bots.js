...
export default function DiscordBots() {
  ...
  const fetchBots = async () => {
    try {
      const res = await api.get('/api/user/discord-bots');
      setBots(res.data);
    } catch (err) {
      console.error(err);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await fetchBots();
      setForm({ token: '', username: '', blocked_words: '' });
    } catch (err) {
      console.error(err);
    } finally {
      setLoading(false);
  ...
  const startBot = async (id) => {
    try {
      await api.post(`/api/user/discord-bots/${id}/start`);
      await fetchBots();
    } catch (err) {
      console.error(err);
    }
  ...
  const stopBot = async (id) => {
    try {
      await api.post(`/api/user/discord-bots/${id}/stop`);
      await fetchBots();
    } catch (err) {
      console.error(err);
    }
    ...
    <Layout title="Discord Bots">
      <div className="grid md:grid-cols-2 gap-8">
        <div className="card p-6">
          <h2 className="text-xl font-bold mb-4">Add New Discord Bot</h2>
